<x-admin-layout>
    <x-slot:header>Edit Kategori: {{ $kategori->nama }}</x-slot:header>
        ...
        <div class="card overflow-hidden">
            <div class="px-6 sm:px-8 py-5 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg sm:text-xl font-bold text-slate-800 tracking-tight">Perbarui Klasifikasi</h2>
                        <p class="text-xs text-slate-400 mt-0.5">Ubah informasi kategori « <span class="font-semibold text-slate-500">{{ $kategori->nama }}</span> »</p>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('admin.kategori.update', $kategori) }}" class="px-6 sm:px-8 py-6 sm:py-7 space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 mt-1.5">Slug saat ini: <span class="font-mono font-semibold">/{{ $kategori->slug }}</span> — akan otomatis diperbarui.</p>
